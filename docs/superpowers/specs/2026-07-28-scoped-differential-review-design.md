# scoped-differential-review Design

## Summary

A new skill `scoped-differential-review` that wraps exactly one vendor skill - `differential-review` (unmodified) - and applies self-contained ticket-scope routing to its findings. The skill runs a security review on a PR via `differential-review`, then classifies every finding against a scope map built from a GitHub issue-label universe: IN SCOPE (actionable against the PR), OWNED BY ANOTHER TICKET (deferred, no PR-author action), or GENUINE GAP (propose a new ticket). It generalizes single-ticket scope routing to a set of target tickets the PR implements.

The skill has zero reference to `scoped-tickets`. All scope-routing logic - scope inventory, A/B/C classification, gap rule, gap-ticket template - is self-contained inside this skill.

## Problem

Running `/differential-review` on a PR that implements one or more scoped tickets produces findings without any boundary against the ticket universe. A reviewer can recommend work that belongs to other open tickets, duplicate scope, or introduce entirely new scope for the PR - none of which the vendor skill checks.

Today this is enforced manually with a verbose prompt:

```
$differential-review <PR> ensuring that review scope remains within #4380 and #4343,
and does not duplicate scope for other open tickets in repo:<repo> is:issue state:open
label:<label>. For new findings, carefully consider whether they should be covered in
#4343 or #4380 scope, or another available open ticket, or we should create a whole
new ticket. Please be careful not to introduce entirely new scope for this PR.
```

The goal is to compress this to a single short invocation with the no-duplication, gap-routing, and no-new-scope guarantees as built-in defaults.

## Goals

- Wrap `differential-review` (vendor, unmodified) as the review engine.
- Generalize scope routing from one target ticket to a set of target tickets the PR implements.
- Build a scope map from a GitHub issue-label universe and classify every finding as A/B/C.
- Propose gap tickets using a self-contained template; create them only after one batch approval.
- Zero reference to `scoped-tickets` anywhere in the skill.
- Compress the verbose scope-constraint prompt to a single slash-command invocation.

## Non-goals

- Modify `differential-review` (vendor skill).
- Reference or depend on `scoped-tickets`.
- Post PR review comments, edit the PR, change labels on existing issues, or close issues.
- Define a new security-review methodology; the skill treats `differential-review` as a black box.
- Decide the fix for a finding; the skill routes scope, it does not author fixes.

## Inputs

- `PR` - URL or `owner/repo#N` of the pull request under review.
- `TICKETS` - one or more target ticket numbers in the label universe that this PR implements (e.g. `4343,4380`). Generalizes the single-ticket model to a set.
- `LABEL` - the issue label defining the ticket universe (e.g. `synchronous-composability-demo`).
- `REPO` - the repository containing the labeled issues. Inferred from `TICKETS` when tickets are given as full URLs.
...
## Self-contained scope-routing concepts
...
The "owner must not" list cites adjacent ticket numbers from the scope map so every boundary is unambiguous. This is intentionally simpler than a full Agent Work Ticket - it captures only what is needed to bound scope for a gap discovered during review.
...
2. Do not alter `differential-review`'s methodology, risk classification, or report structure. The skill treats `differential-review` as a black box that emits a findings report.
...
Since this is a Markdown skill wrapping a vendor skill, verification is static checks plus scenario exercises against the written SKILL.md.

### Static checks

```bash
# No reference to scoped-tickets anywhere in the new skill
