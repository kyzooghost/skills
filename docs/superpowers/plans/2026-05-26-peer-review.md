...
**Goal:** Create a `/peer-review` skill that generates evidence-backed draft peer review answers using exhaustive multi-source discovery.

**Architecture:** Single SKILL.md file following the same multi-phase subagent pattern as win-report. No scripts or reference files needed - all logic is procedural instructions in the skill body.

**Tech Stack:** Claude Code skill (Markdown), GitHub CLI, Atlassian MCP, Notion MCP, Slack MCP.

---

### Task 1: Create skill directory and SKILL.md with frontmatter + invocation
...
```bash
mkdir -p skills/peer-review
```

- [ ] **Step 2: Write SKILL.md with frontmatter and invocation section**

Write `skills/peer-review/SKILL.md` with:
...
```bash
head -5 skills/peer-review/SKILL.md
```

Expected: YAML frontmatter with `name: peer-review`

- [ ] **Step 4: Commit**

```bash
git add skills/peer-review/SKILL.md
git commit -m "feat(peer-review): scaffold skill with frontmatter and setup phase"
```

---

### Task 2: Write Phase 1 - Exhaustive Discovery
...
```bash
git add skills/peer-review/SKILL.md
git commit -m "feat(peer-review): add Phase 1 exhaustive discovery"
```

---

### Task 3: Write Phase 2 - Deep Reading
...
```bash
git add skills/peer-review/SKILL.md
git commit -m "feat(peer-review): add Phase 2 deep reading"
```
...
From evidence patterns (NOT PR mechanics - do not evaluate PR size, code style, or similar AI-assisted artifacts), propose 1-3 candidate growth areas. Focus on:
...
```bash
git add skills/peer-review/SKILL.md
git commit -m "feat(peer-review): add Phase 3 triage"
```

---

### Task 5: Write Phase 4 - Questions

**Files:**
- Modify: `skills/peer-review/SKILL.md`

- [ ] **Step 1: Append Phase 4 content to SKILL.md**
...
```bash
git add skills/peer-review/SKILL.md
git commit -m "feat(peer-review): add Phase 4 gap-filling questions"
```

---

### Task 6: Write Phase 5 - Output and writing rules

**Files:**
- Modify: `skills/peer-review/SKILL.md`

- [ ] **Step 1: Append Phase 5, writing rules, and constraints to SKILL.md**
...
Output one answer per review form question. Each answer uses this structure:

### [Question text as provided by user]

1-2 paragraphs of human-sounding prose with specific examples woven in.

---

After all answers, include:
...
```bash
git add skills/peer-review/SKILL.md
git commit -m "feat(peer-review): add Phase 5 output, writing rules, and constraints"
```

---

### Task 7: Install skill locally

**Files:**
- None created in repo (installs to `~/.claude/commands/`)

- [ ] **Step 1: Install the skill locally using the install-local-skill skill**

Run `/install-local-skill` to make `peer-review` available as a slash command locally.

- [ ] **Step 2: Verify installation**

```bash
ls ~/.claude/commands/peer-review.md 2>/dev/null && echo "INSTALLED" || echo "NOT FOUND"
```

Expected: `INSTALLED`

- [ ] **Step 3: Commit any repo changes from install**

```bash
git status
# If install-local-skill made repo changes, commit them
```
