# scoped-differential-review Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a new skill `skills/scoped-differential-review/SKILL.md` that wraps the vendor `differential-review` skill and applies self-contained ticket-scope routing to its findings, with zero reference to `scoped-tickets`.

**Architecture:** A single new SKILL.md file. The skill instructs the agent to follow `differential-review`'s methodology on a PR, then classify every finding against a scope map built from a GitHub issue-label universe (A: IN SCOPE, B: OWNED BY ANOTHER TICKET, C: GENUINE GAP), emit a scope-adjusted verdict alongside the preserved vendor verdict, and propose gap tickets using a self-contained template with batch approval before filing. All `gh` commands are inline; no scripts.

**Tech Stack:** Markdown, GitHub CLI (`gh`), `rg`, Git

**Design:** `docs/superpowers/specs/2026-07-28-scoped-differential-review-design.md`
...
- Zero reference to `scoped-tickets` anywhere in `skills/scoped-differential-review/SKILL.md`.
- Do not modify the vendor `differential-review` skill (it lives in `~/.agents/skills/differential-review/`, outside this repo).
...
- Never post PR review comments, edit the PR, change labels on existing issues, or close issues.

---

### Task 1: Scaffold SKILL.md with frontmatter, inputs, defaults, and self-contained scope-routing concepts

**Files:**
- Create: `skills/scoped-differential-review/SKILL.md`
- Reference: `docs/superpowers/specs/2026-07-28-scoped-differential-review-design.md`

**Interfaces:**
- Produces: `skills/scoped-differential-review/SKILL.md` containing frontmatter, the Summary, Inputs, Built-in defaults, Scope inventory, A/B/C finding classification, Gap rule, and Self-contained gap-ticket template sections. Later tasks append Workflow, Recommendation reconciliation, Error handling, Invocation, and Verification sections to this same file.

- [ ] **Step 1: Create the directory and write the frontmatter + Summary + Inputs + Built-in defaults**

Create `skills/scoped-differential-review/SKILL.md` with this exact content:

````markdown
---
name: scoped-differential-review
description: Wraps the differential-review skill to scope its findings to a set of target tickets in a GitHub issue-label universe. Use when the user says "/scoped-differential-review" or asks to run a differential review scoped to specific tickets. Classifies every finding as IN SCOPE, OWNED BY ANOTHER TICKET, or GENUINE GAP, and proposes gap tickets. Self-contained - no reference to scoped-tickets.
---
...
- [ ] **Step 2: Append the Scope inventory, A/B/C classification, Gap rule, and gap-ticket template sections**

Append to `skills/scoped-differential-review/SKILL.md`:
...
```bash
rg -n 'name: scoped-differential-review|## Inputs|## Built-in defaults|## Scope inventory|## A/B/C finding classification|## Gap rule|## Self-contained gap-ticket template' skills/scoped-differential-review/SKILL.md
```
...
git commit -m "feat(scoped-differential-review): scaffold skill with scope-routing concepts"
```

Expected: one commit creating `skills/scoped-differential-review/SKILL.md`.

---

### Task 2: Add the Workflow (Phases 1-5) and Recommendation reconciliation sections

**Files:**
- Modify: `skills/scoped-differential-review/SKILL.md` (append after the gap-ticket template section)

**Interfaces:**
- Consumes: the Scope inventory, A/B/C classification, and Gap rule defined in Task 1.
- Produces: the Workflow and Recommendation reconciliation sections that later tasks' Error handling and Verification sections reference.
...
- [ ] **Step 2: Append the Recommendation reconciliation section**
...
- [ ] **Step 3: Verify Task 2 content is present**

Run:

```bash
rg -n '## Workflow|### Phase 1|### Phase 2|### Phase 3|### Phase 4|### Phase 5|## Recommendation reconciliation|Scope-adjusted verdict|blocking-in-scope|blocking-deferred' skills/scoped-differential-review/SKILL.md
```
...
git add skills/scoped-differential-review/SKILL.md
git diff --cached --check
git commit -m "feat(scoped-differential-review): add workflow and recommendation reconciliation"
```

Expected: one commit appending the Workflow and Recommendation reconciliation sections.
...
**Files:**
- Modify: `skills/scoped-differential-review/SKILL.md` (append after Recommendation reconciliation)

**Interfaces:**
- Consumes: Workflow phases and Recommendation reconciliation from Task 2; scope-routing concepts from Task 1.
...
```bash
rg -n '## Error handling and edge cases|## Invocation|## Verification|### Static checks|### Scenario exercises' skills/scoped-differential-review/SKILL.md
```
...
git add skills/scoped-differential-review/SKILL.md
git diff --cached --check
git commit -m "feat(scoped-differential-review): add error handling, invocation, verification"
```
...
**Files:**
- Verify: `skills/scoped-differential-review/SKILL.md`
- Verify: `docs/superpowers/specs/2026-07-28-scoped-differential-review-design.md`
- Verify: `docs/superpowers/plans/2026-07-28-scoped-differential-review.md`
...
# differential-review is referenced (the wrapped skill)
rg -n 'differential-review' skills/scoped-differential-review/SKILL.md
# Expected: matches in description, Phase 3, and Error handling
...
rg -n 'gh issue create|gh issue list|gh issue view|gh pr' skills/scoped-differential-review/SKILL.md
...
```bash
rg -n '## Summary|## Problem|## Goals|## Non-goals|## Inputs|## Built-in defaults|## Self-contained scope-routing concepts|## Workflow|## Recommendation reconciliation|## Error handling and edge cases|## Invocation|## Verification|## Success criteria' docs/superpowers/specs/2026-07-28-scoped-differential-review-design.md
rg -n '## Inputs|## Built-in defaults|## Scope inventory|## A/B/C finding classification|## Gap rule|## Self-contained gap-ticket template|## Workflow|## Recommendation reconciliation|## Error handling and edge cases|## Invocation|## Verification' skills/scoped-differential-review/SKILL.md
```

Expected: every spec concept (inputs, defaults, scope inventory, A/B/C, gap rule, gap-ticket template, workflow phases 1-5, recommendation reconciliation, error handling, invocation, verification scenarios) has a corresponding section in SKILL.md.

- [ ] **Step 3: Verify the scope-adjusted verdict computation is consistent**

Run:

```bash
rg -n 'blocking-in-scope|blocking-deferred|Scope-adjusted verdict|APPROVE|CONDITIONAL|REJECT' skills/scoped-differential-review/SKILL.md
```

Expected: the verdict computation rules (APPROVE/CONDITIONAL/REJECT) and the blocking-in-scope / blocking-deferred definitions appear in the Recommendation reconciliation section, and the scope-adjusted verdict appears in the Phase 5 Scope Routing section.

- [ ] **Step 4: Verify all 13 scenarios from the spec are present in SKILL.md**

Run:

```bash
rg -n 'Single target ticket|Multi target tickets|Finding owned by another open ticket|PR implements another ticket|Genuine gap, batch approval granted|Genuine gap, batch approval declined|gh issue create fails mid-batch|Empty label universe|Target ticket not in the label universe|Target ticket has no parseable scope section|Two target tickets both appear to own|differential-review emits no report|Vendor verdict conflicts with scope-adjusted' skills/scoped-differential-review/SKILL.md
```

Expected: 13 matches, one per scenario.

- [ ] **Step 5: Verify the SKILL.md reads as a coherent standalone skill**

Read `skills/scoped-differential-review/SKILL.md` in full and confirm:

1. The frontmatter `name` and `description` are present and accurate.
2. The sections flow in a logical order: frontmatter -> Summary -> Inputs -> Built-in defaults -> Scope inventory -> A/B/C classification -> Gap rule -> gap-ticket template -> Workflow (Phases 1-5) -> Recommendation reconciliation -> Error handling -> Invocation -> Verification.
...
- The branch changes only `docs/superpowers/specs/2026-07-28-scoped-differential-review-design.md`, `docs/superpowers/plans/2026-07-28-scoped-differential-review.md`, and `skills/scoped-differential-review/SKILL.md`.
- The branch is clean.

- [ ] **Step 7: Commit the plan document**

```bash
git add docs/superpowers/plans/2026-07-28-scoped-differential-review.md
git diff --cached --check
git commit -m "docs(scoped-differential-review): implementation plan"
```
