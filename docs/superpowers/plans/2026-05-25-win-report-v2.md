# Win Report v2 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rewrite the win-report skill to use exhaustive two-phase subagent architecture that paginates all sources and reads all artifacts in full before synthesizing.

**Architecture:** Replace the current single-pass discovery with a two-phase approach: Phase 1 spawns parallel discovery subagents (one per source) that paginate until exhausted. Phase 2 spawns parallel deep-read subagents (batches of 10-15 artifacts) that read full content. Main agent then synthesizes from rich signal summaries.

**Tech Stack:** Claude Code skill (Markdown), GitHub CLI (`gh`), Atlassian MCP tools, Notion MCP tools, Slack MCP tools.

---

### Task 1: Rewrite SKILL.md with new two-phase architecture

**Files:**
- Modify: `skills/win-report/SKILL.md` (full rewrite)

- [ ] **Step 1: Replace SKILL.md with new content**

Write the complete new skill file:
...
- [ ] **Step 2: Verify the new SKILL.md against the spec**

Read both files and confirm:
- Input model matches spec (github_user, github_org, date_range, mcps, hints)
- Two-phase architecture is clearly described with explicit subagent instructions
- Guardrails are present ("paginate until exhausted", "read full content")
- Output format unchanged from original
- Writing rules and constraints preserved
- No placeholders or TBDs

- [ ] **Step 3: Commit**

```bash
git add skills/win-report/SKILL.md
git commit -m "feat(win-report): rewrite with exhaustive two-phase subagent architecture

Replace single-pass shallow discovery with:
- Auto-discover repos from GitHub org activity (no manual repo lists)
- Phase 1: parallel discovery subagents paginate all sources until exhausted
- Phase 2: parallel deep-read subagents read full content of every artifact
- Explicit guardrails: 'paginate until no more results', 'read full body'

Input simplified to github_user + github_org + which MCPs to use.
