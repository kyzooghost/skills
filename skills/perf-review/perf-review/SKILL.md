...
6. **Self-review (per paragraph).** Before presenting the draft, check each paragraph against:
   - Attribution: does "I owned / led / drove" overstate your role? If someone else owned the risk or direction, reattribute.
   - Exclusivity: does "the only / sole / first" hold literally? Default to softer framing.
   - Evidence: are all links openable by the reader? No DMs, no private channels.
   - Hedging: is anything described as "potential" that is already live?
   - Stakes: are pressures described generically? Name the specific constraint.
   - Impact: is business impact left as an abstract noun when a concrete magnitude is defensible?
   - Prior cycle: does the draft connect back to growth areas from the previous review?
